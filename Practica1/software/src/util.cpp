...
void mostrarUso(char *nombre_programa){
	...
	cerr << "Uso (algoritmo Busqueda Local): " << nombre_programa << " " << PARAMETRO_BUSQUEDA_LOCAL << " conjunto porcentaje_restricciones n_repeticiones resultado [semillas]" << endl << endl;
	cerr << "Conjunto: iris -> " << PARAMETRO_IRIS << " ecoli -> " << PARAMETRO_ECOLI << " rand -> " << PARAMETRO_RAND << endl;
	cerr << "Porcentaje restricciones: 10 20" << endl;
	cerr << "Resultado: mostrar -> " << PARAMENTRO_MOSTRAR << " no mostrar -> " << PARAMENTRO_NO_MOSTRAR <<endl << endl;
}
