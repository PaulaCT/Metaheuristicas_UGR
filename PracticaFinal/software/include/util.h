...
const std::string PARAMETRO_CAT_SWARM_OPTIMIZATION = "CSO";
const std::string PARAMETRO_CAT_SWARM_OPTIMIZATION_MEMTICO = "CSO-M";
const std::string PARAMETRO_CAT_SWARM_OPTIMIZATION_PANTERA = "CSO-P";
...
enum Modo{GREEDY,BUSQUEDA_LOCAL,B_MULTIARRANQUE_BASICA, E_SIMULADO, BL_REITERADA, HIBRIDACION, CSO, CSO_MEMETICO, CSO_PANTERA};
enum Conjunto{IRIS,ECOLI,RAND,NEWTHYROID};
enum Comportamiento{SEEKING,TRACING,CLEANING};
...
void fijarDistancia(double dist);
void fijarRestricciones(size_t res);
void getVariables(double &dist, size_t &res);
...
//Asigna a cada cluster el vector promedio de las instancias de X que los componen y devuelve true si hay cambios
bool centroides_bool(std::vector<Cluster> &c, std::vector<Node> x);
...
//Genera un vector ordenado de tamaño tam y lo devuelve desordenado
std::vector<size_t> vector_desordenado(size_t tam);
