...
std::vector<size_t> Randfromvector(std::vector<size_t> valores, size_t tam){
  ...
  for (size_t i=0; i<tam; i++) random_vector.push_back(valores[Randint(0,valores.size()-1)]);
  return random_vector;
}

std::vector<std::vector<size_t>> Randfromvector(std::vector<std::vector<size_t>> valores, size_t tam){
  std::vector<std::vector<size_t>> random_vector;
  for (size_t i=0; i<tam; i++) random_vector.push_back(valores[Randint(0,valores.size()-1)]);
  return random_vector;
}
