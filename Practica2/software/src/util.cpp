...
Modo comprobarParametros(int num_parametros, char **parametros){
	if(num_parametros >= NUM_PARAMETROS){
		...
		if(parametros[1] == PARAMETRO_GENETICO_GENERACIONAL_UN) return GENETICO_GENERACIONAL_UN;
		if(parametros[1] == PARAMETRO_GENETICO_GENERACIONAL_SF) return GENETICO_GENERACIONAL_SF;
		if(parametros[1] == PARAMETRO_GENETICO_ESTACIONARIO_UN) return GENETICO_ESTACIONARIO_UN;
		if(parametros[1] == PARAMETRO_GENETICO_ESTACIONARIO_SF) return GENETICO_ESTACIONARIO_SF;
		if(parametros[1] == PARAMETRO_MEMETICO_10) return MEMETICO_10;
		if(parametros[1] == PARAMETRO_MEMETICO_01) return MEMETICO_01;
		if(parametros[1] == PARAMETRO_MEMETICO_MEJOR) return MEMETICO_MEJOR;
		if(parametros[1] == PARAMETRO_MEMETICO_10_1) return MEMETICO_10_1;
		if(parametros[1] == PARAMETRO_MEMETICO_01_1) return MEMETICO_01_1;
		if(parametros[1] == PARAMETRO_MEMETICO_MEJOR_1) return MEMETICO_MEJOR_1;
...
void mostrarUso(char *nombre_programa){
	...
	cerr << "Generico Generacional (cruce uniforme): " << PARAMETRO_GENETICO_GENERACIONAL_UN << endl;
	cerr << "Generico Generacional (cruce por segmento fijo): " << PARAMETRO_GENETICO_GENERACIONAL_SF << endl;
	cerr << "Generico Estacionario (cruce uniforme): " << PARAMETRO_GENETICO_ESTACIONARIO_UN  << endl;
	cerr << "Generico Estacionario (cruce por segmento fijo): " << PARAMETRO_GENETICO_ESTACIONARIO_SF << endl;
	cerr << "Memetico-(10,1.0): " << nombre_programa << " " << PARAMETRO_MEMETICO_10 << endl;
	cerr << "Memetico-(10,0.1): " << nombre_programa << " " << PARAMETRO_MEMETICO_01 << endl;
	cerr << "Memetico-(10,0.1mej): " << nombre_programa << " " << PARAMETRO_MEMETICO_MEJOR << endl << endl;
	cerr << "Memetico-(1,1.0): " << nombre_programa << " " << PARAMETRO_MEMETICO_10_1 << endl;
	cerr << "Memetico-(1,0.1): " << nombre_programa << " " << PARAMETRO_MEMETICO_01_1 << endl;
	cerr << "Memetico-(1,0.1mej): " << nombre_programa << " " << PARAMETRO_MEMETICO_MEJOR_1 << endl << endl;
	cerr << "--- Conjuntos ---" << endl;
	...
	while (diferencias){
		...
		for(size_t i = 0; i < num_elem; i++){
			...
			for(size_t j = 0; j < num_clus; j++){
				restricciones = probarCluster(C,X,M,i,j,contador);
...
int probarCluster(vector<Cluster> c, vector<Node> x, vector<vector<int>> m, size_t nodo, size_t clus, size_t iteracion){
	if (iteracion > 1){
		...
		for(vector<size_t>::iterator it=c[x[nodo].cluster].nodos.begin();buscando;it++){
			if((*it)==nodo){
				posicion=it;
				buscando=false;
			}
		}
		c[x[nodo].cluster].nodos.erase(posicion);
	}
